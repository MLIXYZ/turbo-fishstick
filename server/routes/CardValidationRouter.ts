...
function validateCard(body: ValidateCardBody): ValidationError[] {
    ...
    if (!nameOnCard)
        errors.push({ field: 'nameOnCard', message: 'Name is required' })
    ...
    if (!cvvDigits) {
        ...
        if (cvvDigits.length !== 4)
            errors.push({ field: 'cvv', message: 'AMEX CVV must be 4 digits' })
    } else {
        if (cvvDigits.length !== 3)
            errors.push({ field: 'cvv', message: 'CVV must be 3 digits' })
    }
